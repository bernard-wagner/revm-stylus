...
use revm::primitives::{bytes, keccak256, Address, Bytes, B256, U256};
...
use crate::interpreter::EthOrStylusInterpreter;
...
pub struct ArbOsFrame<CTX, ERROR, PRECOMPILE, INSTRUCTIONS> {
    ...
    /// Interpreter.
    pub interpreter: EthOrStylusInterpreter,
    ...
    CTX: JournalStateGetter,
{
    pub fn new(
        data: FrameData,
        depth: usize,
        interpreter: EthOrStylusInterpreter,
...
where
    CTX: ArbOsFrameContext<ERROR>,
    ERROR: ArbOsFrameError<CTX>,
    PRECOMPILE: PrecompileProvider<Context = CTX, Error = ERROR>,
{
    ...
    pub fn make_call_frame(
        ...
        if let Some(result) = precompile.run(
            ...
            let interpreter = if bytecode.bytes().starts_with(&STYLUS_MAGIC_BYTES) {
                EthOrStylusInterpreter::Stylus(Box::new(StylusInterpreter::new(
                    bytecode,
                    interpreter_input,
                    false,
                    inputs.gas_limit,
                ...
                EthOrStylusInterpreter::Eth(Box::new(Interpreter::new(
                    memory.clone(),
                    bytecode,
                    interpreter_input,
                    inputs.is_static,
    ...
    pub fn make_create_frame(
        ...
        println!("arbos make_create_frame");
    ...
    pub fn init_with_context(
        ...
        println!("arbos init_with_context");
        match frame_init {
    ...
    INSTRUCTION: InstructionProvider<WIRE = EthInterpreter<()>, Host = CTX>,
{
    ...
    fn run(
        ...
        println!("Running ArbOS frame");
        let spec = context.cfg().spec().into();

        // run interpreter
        let next_action = match self.interpreter {
            EthOrStylusInterpreter::Eth(ref mut interpreter) => {
                println!("Running Eth interpreter");
                interpreter.run(self.instructions.table(), context)
            }
            EthOrStylusInterpreter::Stylus(ref mut interpreter) => {
                println!("Running Stylus interpreter");
    ...
    fn return_result(
        ...
        let interpreter = match &mut self.interpreter {
            EthOrStylusInterpreter::Eth(interpreter) => interpreter,
            EthOrStylusInterpreter::Stylus(stylus_interpreter) => {
                stylus_interpreter.return_result(result);
...
pub trait ArbOsFrameContext<ERROR>:
...
impl<
        ...
        CTX: TransactionGetter
            ...
            + CfgGetter
            + Host,
    > ArbOsFrameContext<ERROR> for CTX
{
}

pub trait ArbOsFrameError<CTX: JournalStateGetter>:
