...
use revm::interpreter::{CallOutcome, InterpreterResult};
        ...
        match self.rx.recv() {
            Ok(response) => match response {
                ...
                EvmApiOutcome::Create(stylus_outcome, address, gas_cost) => {
                    ...
                    (result, VecReader::new(data.to_vec()), Gas(gas_cost))
                }
...
pub fn stylus_call(
    ...
    let ink_limit = config.pricing.gas_to_ink(Gas(gas_limit));
    ...
    thread::spawn(move || {
        ...
        let mut gas = revm::interpreter::Gas::new(gas_limit);
        gas.spend_all();
        ...
        let outcome = match out_kind {
            ...
            UserOutcomeKind::OutOfStack => revm::interpreter::InstructionResult::CallTooDeep,
        };
        ...
        instance
            ...
            .request_handler()
            .tx
            .send(EvmApiRequest::Return(outcome, 0))
            .unwrap();
    });
}
